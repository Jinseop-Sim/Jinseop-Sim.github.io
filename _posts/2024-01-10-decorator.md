...
### 장단점
Decorator 패턴의 장점은 다음과 같다.  
- 여러 동작의 조합을 매우 유연하게 구현할 수 있다.
- 각 장식 당 하나의 책임을 부여해 ```SRP```를 준수한다.
- 구현체가 아닌 interface에 의존하므로 ```DIP```를 준수한다.  

아래는 Decorator 패턴의 단점이다.
- 생성자들을 감쌌을 때, 중간에 있는 일부만 제거하기가 어렵다.
- 생성자가 결합된 Wrapper 구조가 깔끔하지 않을 수도 있다.
