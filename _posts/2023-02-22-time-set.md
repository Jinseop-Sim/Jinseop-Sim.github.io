...
이 사실과 Postman에 제대로 출력이 안되는게 연관이 있을 것 같다.  
...
응답 시, ```Jackson```의 ```ObjectMapper```가 동작해서 Json으로 변환을 하는데,  
...
이 방식을 이용하면 응답에서의 출력도 제대로 동작할 것 같다.  

토큰 만료에 대한 기능도 잘 작동하고, 시간 관련 기능들도 잘 동작하니 다행이다.  
공식문서를 찾아보는 경험치만 +1 된 것 같은 기분이다.  

#### 참고문헌
> [MDN HTTP](https://developer.mozilla.org/ko/docs/Web/HTTP/Headers/Date)  
> [RFC 9110 HTTP](https://httpwg.org/specs/rfc9110.html#http.date)  
